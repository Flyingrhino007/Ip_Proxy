...
class WebRequest(object):
    ...
    @property
    def user_agent(self):               # Property属性，可以直接返回一个值
        """
    ...
    def get(self, url, header=None, retry_time=5, timeout=30,
        ...
        """
        headers = self.header
        if header and isinstance(header, dict):     # 若函数输入有附加 header，要更新到headers内
            headers.update(header)
        while True:
            try:
                html = requests.get(url, headers=headers, timeout=timeout, **kwargs)
                # any() 函数用于判断给定的可迭代参数 iterable 是否全部为 False，则返回 False，如果有一个为 True，则返回 True。
                if any(f in html.content for f in retry_flag):  # 若retry_flag中有任何一个在html.content内容中，即raise Exception
                    raise Exception
                return html
                ...
                if retry_time <= 0:
                    ...
                    return resp
                time.sleep(retry_interval)      # 每次失败的尝试间间隔5秒
